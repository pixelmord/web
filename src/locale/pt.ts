...
const translate: TranslationFn = (key, args) => {
  const data: TranslationData = {
    ...
    formatting_label: 'Formatação do texto',
    ...
    no_chapter_examples: `Ajude mais pessoas a entenderem por que é importante aprender sobre **${args?.title}**. [Compartilhe um exemplo prático](/posts/add?category=examples&topicId=${args?.topicId}&chapterId=${args?.chapterId}).`,
    no_chapters: `Os capítulos têm aulas para ensinar um assunto. Ajude mais pessoas a aprenderem sobre **${args?.title}**. [Crie o primeiro capítulo](/chapters/add?topicId=${args?.id}).`,
    ...
    no_lessons: `Seja o primeiro a [criar uma aula](/posts/add?category=lessons&topicId=${args?.topicId}&chapterId=${args?.chapterId}) e ajude outras pessoas a aprenderem sobre **${args?.title}**.`,
    ...
    teach_intro:
      'O Zoonk é um site gratuito e aberto porque acreditamos que o conhecimento deve ser livre. Quando você compartilha o seu conhecimento, você está ajudando outras pessoas a melhorarem de vida.\n\nVocê pode ensinar de várias formas: desde criando aulas para explicar um assunto que você conhece até maneiras mais simples como compartilhando um curso ou artigo que você achou interessante.\n\nVeja abaixo como começar:',
    teach_lesson_desc: `Muitas pessoas não sabem por onde começar a aprender. Crie aulas para ajudá-las nesse caminho. Algumas coisas que você pode fazer:\n- Veja se já existem alguns [capítulos para ${args?.title}](/topics/${args?.id}). Caso contrário, comece criando um capítulo introdutório para ensinar sobre o assunto que desejar.\n- Cada capítulo pode ter até 20 aulas. Crie novas ou melhore as que já existem. O Zoonk é colaborativo: qualquer um pode melhorar as aulas existentes.`,
